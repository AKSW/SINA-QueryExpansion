...
public class MainWebPage implements ClickListener {
	...
	Label ExampleLable1 = new Label("<b>Example 1:</b>  <i> What are the side effects of Valdecoxib ?</i>");
	Label ExampleLable2 = new Label("<b>Example 2:</b>  <i>Which drug does have fever as side effect?</i>");
	Label ExampleLable3 = new Label("<b>Example 3:</b>  <i>Which are the drugs whose side effects are associated with the gene TRPM6?</i>");
	...
	public void showMainWebPage()
	{
		...
		Embedded InterlinkedLogo = new Embedded("",
                new ThemeResource("Interlinkeddatasets-logo.jpg"));
		...
		panelQuery.addComponent(InterlinkedLogo);
        ...
        layout.setComponentAlignment(InterlinkedLogo, Alignment.TOP_RIGHT);
	...
	private  void searchButtonClicked() {
		...
		//	System.out.println("  plimmer stemmer ..... " + new PlingStemmer().stem("people"));
		 ...
		 for(int  i=0 ; i<c.getquery_list().size() ; i++)
		 {
			 ...
			 System.out.println(" sparql in gui" + s2);
			 Link l = new Link("Show the query in SPARQL endpoint",
		                new ExternalResource("http://139.18.2.96:8910/sparql?default-graph-uri=&qtxt="+s2));
			 // new ExternalResource("http://139.18.2.96:8910/sparql?default-graph-uri=http%3A%2F%2Fdbpedia.org&qtxt"+s2));
